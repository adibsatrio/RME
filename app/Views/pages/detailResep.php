...
<div class='d-flex gap-3 mt-5 border rounded-3 bg-white p-3'>
    ...
    <div class="w-100">
        ...
        <div
            ...
            <div>
                <?php if($rekmed['status_kunjungan'] == 'selesai') : 
                    $end = format_time($rekmed['updated_at']); ?>
                <p class="mb-0 bg-success p-1 rounded-3 fw-medium text-white">
                    <?= format_time($rekmed['created_at']) ?> -
                    <?= $end ?></p>
                <?php else : ?>
                <p class="mb-0 bg-success p-1 rounded-3 fw-medium text-white"><?= format_time($rekmed['created_at']) ?>
                    - PROSES
                </p>
                <?php endif; ?>
            </div>
